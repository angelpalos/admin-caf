function venta_dia(req, res) {
    ...
    req.getConnection((err, conn) => {
        conn.query("SELECT b.fecha, SUM(a.cantidad * c.precio) AS total FROM detalle a JOIN pedido b ON a.folio = b.folio JOIN product c ON c.id_producto = a.id_producto WHERE b.fecha = date(?) and b.id_status=5 GROUP BY b.fecha ORDER BY b.fecha DESC;", [data.fecha], (err, rep_dia) => {
            if (rep_dia.length > 0) {
                reporte_dia = [{
                    fecha:data.fecha,
...
function venta_periodo(req,res){
    ...
    req.getConnection((err, conn)=>{
        conn.query("SELECT DATE_FORMAT(a.fecha, '%d-%m-%Y') AS fecha ,SUM(b.cantidad*b.precio) AS total FROM pedido a, detalle b WHERE b.folio=a.folio and a.fecha>=date(?) AND a.fecha<=date(?) AND a.id_status=5 GROUP BY fecha ORDER BY fecha DESC",[data.fecha_i,data.fecha_f],(err,rep_periodo)=>{
            if (err) throw err;
...
function trabajador_dia(req, res) {
    ...
    req.getConnection((err, conn) => {
        conn.query("SELECT DATE_FORMAT(a.fecha, '%d-%m-%Y') AS fecha, a.folio, a.correo_clie,b.name,b.ap_paterno FROM pedido a, users b WHERE a.id_status=5 AND a.fecha=date(?) AND b.email=a.corre_emp GROUP BY a.fecha ORDER BY a.fecha DESC", [data.fecha], (err, reporte_dia) => {
            if (err) throw err;
...
function trabajador_periodo(req, res) {
    ...
    req.getConnection((err, conn) => {
        conn.query("SELECT DATE_FORMAT(a.fecha, '%d-%m-%Y') AS fecha, a.folio, a.correo_clie,b.name,b.ap_paterno FROM pedido a, users b WHERE a.id_status=5 AND a.fecha>=date(?) AND a.fecha<=date(?) AND b.email=a.corre_emp GROUP BY a.fecha ORDER BY a.fecha DESC;", [data.fecha_i, data.fecha_f], (err, reporte_periodo) => {
            if (err) throw err;
...
function detalle_dia(req, res) {
    ...
    req.getConnection((err, conn) => {
        conn.query("SELECT DATE_FORMAT(b.fecha, '%d-%m-%Y') AS fecha, b.folio, b.correo_clie, a.name FROM detalle a JOIN pedido b ON a.folio = b.folio JOIN product c ON c.id_producto = a.id_producto WHERE b.fecha=date(?) AND b.id_status=5 GROUP BY a.folio, a.name ORDER BY b.fecha DESC;", [data.fecha], (err, reporte_dia) => {
            if (err) throw err;
            if (reporte_dia.length > 0) {
                //console.log(reporte_periodo);
...
function detalle_periodo(req, res) {
    ...
    req.getConnection((err, conn) => {
        conn.query("SELECT DATE_FORMAT(b.fecha, '%d-%m-%Y') AS fecha, b.folio, b.correo_clie, a.name FROM detalle a JOIN pedido b ON a.folio = b.folio JOIN product c ON c.id_producto = a.id_producto WHERE b.fecha>=date(?) AND b.fecha<=date(?) AND b.id_status=5 GROUP BY a.folio, a.name ORDER BY b.fecha DESC;", [data.fecha_i, data.fecha_f], (err, reporte_periodo) => {
            if (err) throw err;
            if (reporte_periodo.length > 0) {
                //console.log(reporte_periodo);
